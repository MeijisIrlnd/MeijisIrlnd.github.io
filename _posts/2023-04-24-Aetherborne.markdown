...
<br>
<i><sub>side note: from euler's formula, `exp(j0) = cos(0) + j sin(0)`, in the case of the complex exponential, so `exp(-jwt) = cos(wt) - jsin(wt)`, there's an argument to be made for writing the entire stft out using sin and cos instead of euler's number, but I digress..</sub></i>
<br>
<br>
...
![img_6.png](img_6.png)
<br>
and now we're sliding the <i>signal</i> along, while the window is held in place. The problem is, now the `exp(...)` term doesn't get adjusted, as technically it should be `exp(-jwkn + m)`. So to compensate, we need to apply a <i>phase twist</i> to our band when we do our processing.
<br><br> 
...
all of this to say that multiplying by `exp(j0)` will <i>spin</i> your value along the unit circle by theta radians. So we're essentially trying to "spin" our bin to DC (the 0th bin's phase) to do our processing, then spin it the exact inverse to get it back to normal. 
<br><br>
